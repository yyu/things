///
/// \file LiquidCrystal_PCF8574_ATtiny85.cpp
...
///
/// ChangeLog see: LiquidCrystal_PCF8574_ATtiny85.h
...
LiquidCrystal_PCF8574_ATtiny85::LiquidCrystal_PCF8574_ATtiny85(uint8_t addr)
{
  _Addr = addr;
  _backlight = 0;
} // LiquidCrystal_PCF8574_ATtiny85


void LiquidCrystal_PCF8574_ATtiny85::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) {
  // cols ignored !
  _numlines = lines;
...
void LiquidCrystal_PCF8574_ATtiny85::clear()
{
  _command(LCD_CLEARDISPLAY);  // clear display, set cursor position to zero
  delayMicroseconds(2000);  // this command takes a long time!
}

void LiquidCrystal_PCF8574_ATtiny85::home()
{
...
void LiquidCrystal_PCF8574_ATtiny85::setCursor(uint8_t col, uint8_t row)
{
...
void LiquidCrystal_PCF8574_ATtiny85::noDisplay() {
  _displaycontrol &= ~LCD_DISPLAYON;
  _command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void LiquidCrystal_PCF8574_ATtiny85::display() {
  _displaycontrol |= LCD_DISPLAYON;
  _command(LCD_DISPLAYCONTROL | _displaycontrol);
}

// Turns the underline cursor on/off
void LiquidCrystal_PCF8574_ATtiny85::noCursor() {
  _displaycontrol &= ~LCD_CURSORON;
  _command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void LiquidCrystal_PCF8574_ATtiny85::cursor() {
  _displaycontrol |= LCD_CURSORON;
  _command(LCD_DISPLAYCONTROL | _displaycontrol);
}

// Turn on and off the blinking cursor
void LiquidCrystal_PCF8574_ATtiny85::noBlink() {
  _displaycontrol &= ~LCD_BLINKON;
  _command(LCD_DISPLAYCONTROL | _displaycontrol);
}
void LiquidCrystal_PCF8574_ATtiny85::blink() {
  _displaycontrol |= LCD_BLINKON;
  _command(LCD_DISPLAYCONTROL | _displaycontrol);
}

// These commands scroll the display without changing the RAM
void LiquidCrystal_PCF8574_ATtiny85::scrollDisplayLeft(void) {
  _command(LCD_CURSORSHIFT | LCD_DISPLAYMOVE | LCD_MOVELEFT);
}
void LiquidCrystal_PCF8574_ATtiny85::scrollDisplayRight(void) {
  _command(LCD_CURSORSHIFT | LCD_DISPLAYMOVE | LCD_MOVERIGHT);
}

// This is for text that flows Left to Right
void LiquidCrystal_PCF8574_ATtiny85::leftToRight(void) {
  _displaymode |= LCD_ENTRYLEFT;
  _command(LCD_ENTRYMODESET | _displaymode);
}

// This is for text that flows Right to Left
void LiquidCrystal_PCF8574_ATtiny85::rightToLeft(void) {
  _displaymode &= ~LCD_ENTRYLEFT;
  _command(LCD_ENTRYMODESET | _displaymode);
}

// This will 'right justify' text from the cursor
void LiquidCrystal_PCF8574_ATtiny85::autoscroll(void) {
  _displaymode |= LCD_ENTRYSHIFTINCREMENT;
  _command(LCD_ENTRYMODESET | _displaymode);
}

// This will 'left justify' text from the cursor
void LiquidCrystal_PCF8574_ATtiny85::noAutoscroll(void) {
  _displaymode &= ~LCD_ENTRYSHIFTINCREMENT;
  _command(LCD_ENTRYMODESET | _displaymode);
}
...
void LiquidCrystal_PCF8574_ATtiny85::setBacklight(uint8_t brightness) {
...
void LiquidCrystal_PCF8574_ATtiny85::createChar(uint8_t location, uint8_t charmap[]) {
...
inline size_t LiquidCrystal_PCF8574_ATtiny85::write(uint8_t value) {
...
inline void LiquidCrystal_PCF8574_ATtiny85::_command(uint8_t value) {
  _send(value, RSMODE_CMD);
} // _command()


// write either command or data
void LiquidCrystal_PCF8574_ATtiny85::_send(uint8_t value, uint8_t mode) {
...
void LiquidCrystal_PCF8574_ATtiny85::_sendNibble(uint8_t halfByte, uint8_t mode) {
...
void LiquidCrystal_PCF8574_ATtiny85::_write2Wire(uint8_t halfByte, uint8_t mode, uint8_t enable) {
